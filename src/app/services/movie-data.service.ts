...
export class MovieDataService {
  public movies = null;
  public movieListUrl =
    'https://api.themoviedb.org/3/list/104297?language=en-US&api_key=5846b2aa98474b5a56965508b8016f42';
  ...
  getInfo(moviesArray): Observable<any> {
    ...
    moviesArray.map(movie => {
      observableBatch.push(
        this.http.get(
          `https://api.themoviedb.org/3/movie/${movie.id}?api_key=5846b2aa98474b5a56965508b8016f42&append_to_response=credits`
        )
      );
    });
  ...
  filterAllMovies(data) {
    return data.map(movie => {
      console.log(movie);
      return this.filterSingleMovie(movie);
    });
  }
  ...
  filterSingleMovie(movie) {
    ...
    // for (var i = 2; i >= 0; i--) {
    //   //no more than 3 directors for a film
